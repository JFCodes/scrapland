import type { T_API_RESPONSE_Task_Vehicle_FindNew, T_Task_Ad_Vehicle_FindNew_Patch } from '@scrapland/data-model'
import { DBSchema_Task_Ad_Vehicle_FindNew, E_ENTITY_TYPE } from '@scrapland/data-model'
...
export async function controller(req: Request, res: Response<T_API_RESPONSE_Task_Vehicle_FindNew>) {
  ...
  const payload = req.body as T_Task_Ad_Vehicle_FindNew_Patch

  const patched = await db
    .update(DBSchema_Task_Ad_Vehicle_FindNew)
    .set(payload)
    .where(eq(DBSchema_Task_Ad_Vehicle_FindNew._id, taskId))
    .returning()
